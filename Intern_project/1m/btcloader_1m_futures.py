...
import time
from schemas import Candle1mFut,TechInd1mFut
from sqlalchemy import create_engine, select, Table, MetaData
from sqlalchemy.orm import sessionmaker
from sqlalchemy.exc import SQLAlchemyError

# Устанавливаем опцию, чтобы выводить все столбцы
...
logging.basicConfig(filename='btcloader_1m_futures.log', level=logging.INFO,
...
metadata = MetaData()
candlestick_table = Table('candles_1m_fut', metadata, autoload_with=engine)
...
def get_last_timestamp():
    try:
        # Получаем последний timestamp из базы данных
        last_candle = session.query(Candle1mFut).order_by(Candle1mFut.timestamp.desc()).first()
        if last_candle:
...
def fetch_and_calculate():
    ...
    if not df_new.empty:
        ...
        print(f"Data updated, new rows: {len(df_new)}, from previous date in database: {last_timestamp}")
    #else:
     #   print("No new data to append.")
# Планирование задачи каждые 30 минут
schedule.every(5).minutes.do(fetch_and_calculate)
